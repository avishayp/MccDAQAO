...
namespace MccDAQWrapper
{
    public class clsAnalogIO
    {
        ...
        public const int DATAEVENT = 256;
        public const int ENDEVENT = 512;
        public const int PRETRIGEVENT = 1024;
        public const int ERREVENT = 2048;
            ...
            out int DefaultChan, out MccDaq.TriggerType DefaultTrig)
        {
            ...
            bool CheckOutputEvents = false;
            bool CheckInputEvents = false;
            ...
            switch (IOType)
            {
                case ANALOGINPUT:
                    ...
                    if (ChansFound > 0)
                    {
                        ...
                        if (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors)
                            Resolution = ADRes;
                        if ((AnalogType & 0xF00) > 0) CheckInputEvents = true;
                ...
                default:
                    ...
                    if (ChansFound > 0)
                    {
                        ...
                        Resolution = DARes;
                        if ((AnalogType & 0xF00) > 0) CheckOutputEvents = true;
            ...
            if ((ChansFound > 0) & (CheckInputEvents | CheckOutputEvents))
            {
                // check support of event handling by trial 
                // and error with error handling disabled
                bool EventsSupported = false;
                if ((AnalogType & ERREVENT) > 0)
                {
                    ULStat = DaqBoard.DisableEvent(MccDaq.EventType.OnScanError);
                    EventsSupported = (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors);
                }
                if (CheckInputEvents & EventsSupported)
                {
                    if ((AnalogType & DATAEVENT) > 0)
                    {
                        ULStat = DaqBoard.DisableEvent(MccDaq.EventType.OnDataAvailable);
                        EventsSupported = (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors);
                    }
                    if (EventsSupported & (AnalogType & ENDEVENT) > 0)
                    {
                        ULStat = DaqBoard.DisableEvent(MccDaq.EventType.OnEndOfAiScan);
                        EventsSupported = (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors);
                    }
                    if (EventsSupported & (AnalogType & PRETRIGEVENT) > 0)
                    {
                        ULStat = DaqBoard.DisableEvent(MccDaq.EventType.OnPretrigger);
                        EventsSupported = (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors);
                    }
                }
                if (EventsSupported & CheckOutputEvents)
                {
                    if ((AnalogType & ENDEVENT) > 0)
                    {
                        ULStat = DaqBoard.DisableEvent(MccDaq.EventType.OnEndOfAoScan);
                        EventsSupported = (ULStat.Value == MccDaq.ErrorInfo.ErrorCode.NoErrors);
                    }
                }
                if (!EventsSupported) ChansFound = 0;
            }
